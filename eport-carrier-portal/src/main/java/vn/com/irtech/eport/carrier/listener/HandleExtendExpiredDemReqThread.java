...
public class HandleExtendExpiredDemReqThread {
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					try {
						// Get all edo
						// When 1 edo put in queue, the thread will be trigger and get 1 edo
						// => Need to wait if has any edo can be push in queue in the mean time
						...
						// Get remaining edo has been pushed in queue in the time waiting
						List<Edo> edos2 = queueService.getAllEdoExtendExpiredDem();
						// Add list edo get in second times to list edos
						if (CollectionUtils.isNotEmpty(edos2)) {
							edos.addAll(edos2);
						}
						// Check if list is not empty
						if (CollectionUtils.isNotEmpty(edos)) {
							// Sort edos by job order no to get separate edo with the same job order no
							...
							for (Edo edo : edos) {
								if (!edo.getJobOrderNo().equalsIgnoreCase(currentJobOrderNo)) {
									...
									// Send list to next function (Remove edo with same container and send to robot)
	...
	private void filterDuplicateContainer(List<Edo> edos) {
		// Sort edos by container number for easy to remove edo with same container no
		Collections.sort(edos, new ContainerNoComparator());
		// List edos to push on edo list with different container no and has last update
		// time
		List<Edo> edosResult = new ArrayList<Edo>();
		...
		for (Edo edo : edos) {
			...
			if (currentUpdateTime == null) {
				...
				edosResult.remove(edosResult.size() - 1);
				edosResult.add(edo);
				currentUpdateTime = edo.getUpdateTime();
			}
		}
		sendReqToRobot(edosResult);
	}
