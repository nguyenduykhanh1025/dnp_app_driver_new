...
(function ($) {
  ...
  $.fn.bootstrapTreeTable = function (options, param) {
    ...
    var initToolbar = function () {
      ...
      if (options.showSearch) {
        var $searchBtn = $('<button class="btn btn-default btn-outline" type="button" aria-label="search" title="Search for"><i class="glyphicon glyphicon-search"></i></button>');
      ...
      if (options.showColumns) {
        var $columns_div = $(
          '<div class="btn-group pull-right" title="Column"><button type="button" aria-label="columns" class="btn btn-default btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="glyphicon glyphicon-list"></i> <span class="caret"></span></button></div>'
        );
          ...
          if (_isExpanded || _isCollapsed) {
            ...
            if (_isExpanded) {
              ...
              if (_ls && _ls.length > 0) {
                $.each(_ls, function (index, item) {
                  // 父icon
                  var _p_icon = $("#" + $(item).attr("pid"))
                    .children()
                    .eq(options.expandColumn)
                    .find(".treetable-expander");
  ...
  $.fn.bootstrapTreeTable.defaults = {
    code: "code", // select the value returned by the record, used to set the parent-child relationship
    parentCode: "parentCode", // used to set the parent-child relationship
    rootIdValue: null, // Set the root node id value-the root node can be specified, the default is null, "",0,"0"
    data: null, // construct the data collection of the table
    type: "GET", // ajax type of request data
    url: null, // url of ajax requesting data
    ajaxParams: {}, // ajax data attribute of request data
    expandColumn: 0, // On which column to display the expand button
    expandAll: false, // whether to expand all
    expandFirst: true, // Whether to expand the first level by default--effective when expandAll is false
    striped: false, // Whether each line is gradient
    bordered: true, // Whether to display the border
    hover: true, // whether to hover
    condensed: false, // Whether to tighten the table
    columns: [], // columns
    toolbar: null, // top toolbar
    height: 0, // table height
    showTitle: true, // Whether to use the title attribute to display the field content (fields formatted by the formatter will not be displayed)
    showSearch: true, // whether to display search information
    showColumns: true, // Whether to display the content column drop-down box
    showRefresh: true, // whether to show the refresh button
    expanderExpandedClass: "glyphicon glyphicon-chevron-down", // icon of the expanded button
    expanderCollapsedClass: "glyphicon glyphicon-chevron-right", // icon of the expanded button
